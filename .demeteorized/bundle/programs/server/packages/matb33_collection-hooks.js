...
CollectionHooks.defineAdvice("update", function (userId, _super, instance, aspects, getTransform, args, suppressAspects) {
  ...
  if (!suppressAspects) {                                                                                            // 19
    ...
    if (aspects.after) {                                                                                             // 27
      prev.mutator = EJSON.clone(args[1]);                                                                           // 28
      prev.options = EJSON.clone(args[2]);                                                                           // 29
      if (_.some(aspects.after, function (o) { return o.options.fetchPrevious !== false; }) &&                       // 30
          CollectionHooks.extendOptions(instance.hookOptions, {}, "after", "update").fetchPrevious !== false) {      // 31
        prev.docs = {};                                                                                              // 32
